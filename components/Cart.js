...
export default function Cart({ items, onUpdateJumlah, onRemove }) {
  ...
  items.forEach((item) => {
    const label = item.warna ? item.nama + ' (' + item.warna + ')' : item.nama;
    teks += '- ' + label + ' (' + item.ukuran + ') x' + item.jumlah + ' dus\n';
  });
  ...
  return (
    <div className="cart-box">
      ...
      {items.map((item) => (
        <div key={item.produk_id + '-' + item.warna} className="cart-item">
          ...
          <input
            ...
            min="0"
            value={item.jumlah}
            onChange={(e) => onUpdateJumlah(item.produk_id, item.warna, Number(e.target.value))}
            className="cart-item-qty"
          />
          <span className="cart-item-unit">dus</span>
          <button
            onClick={() => onRemove(item.produk_id, item.warna)}
